...
export default function GraphPage() {
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col gap-3 rounded-lg border border-zinc-800/70 bg-zinc-900/40 p-4 backdrop-blur-md md:flex-row md:items-center md:justify-between">
        <button onClick={() => setTimeline((value) => !value)} className="flex items-center gap-2 text-sm text-zinc-300"><Clock className="size-4 text-cyan-200" /> Timeline View <Badge variant={timeline ? "cyan" : "outline"}>{timeline ? "on" : "off"}</Badge></button>
        <div className="flex flex-1 items-center gap-3 md:max-w-lg"><span className="font-mono text-xs text-zinc-500">{days}d</span><input className="w-full accent-cyan-300" type="range" min="1" max="180" value={days} onChange={(e) => setDays(Number(e.target.value))} /></div>
